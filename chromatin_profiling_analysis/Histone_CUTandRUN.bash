...
for f in *_filtered.sam; do if [[ ! -e "${f/_filtered.sam/.1_120.sam}" ]]; then awk ' $9 <= 500 && $9 >= 150 || $9 >= -500 && $9 <= -150 ' $f > ${f/_filtered.sam/.150_500.sam}; cp $header ${f/_filtered.sam/.150_500.header}; cat ${f/_filtered.sam/.150_500.sam} >> ${f/_filtered.sam/.150_500.header}; rm ${f/_filtered.sam/.150_500.sam}; mv ${f/_filtered.sam/.150_500.header} ${f/_filtered.sam/.150_500.sam}; samtools view -@ 7 -S -t $hg38.cs -b -o ${f/_filtered.sam/.150_500.bam} ${f/_filtered.sam/.150_500.sam}; rm ${f/_filtered.sam/.150_500.sam}; fi; done
