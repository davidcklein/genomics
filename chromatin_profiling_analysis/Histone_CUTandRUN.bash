...
for f in *R1_trim25.fastq; do echo $f; if [[ ! -e "${f/_R1_trim25.fastq/.sam}" ]]; then bowtie2 -q -N 1 -p 8 -X 1000 -I 50 --no-unal -x $hg38 -1 $f -2 ${f/R1/R2} -S ${f/_R1_trim25.fastq/.sam}; fi; done 
...
for f in *_dup_marked.bam; do if [[ ! -e "${f/_dup_marked.bam/_filtered.sam}" ]]; then samtools view -@ 7 -h -Sq 10 -o ${f/_dup_marked.bam/_filtered.sam} $f; fi; done
...
samtools view -@ 7 -S -t $hg38.cs -b -o ${f/_filtered.sam/.150_500.bam} ${f/_filtered.sam/.150_500.sam}; rm ${f/_filtered.sam/.150_500.sam}; fi; done
...
#This step will generate a genome coverage bigWig file of all regions covered by this experiment, normalized to 1X coverage and binned in 5-bp segments. 
#I prefer 5-bp bins as a compromise of speed and resolution (particularly given ChIP chromatin shearing limitations), but -bs can be altered down to 1 for high-resolution or up for faster analysis. 
#If -bs is changed, --smoothLength should also be changed to maintain a 1:4 ratio. The --effectiveGenomeSize parameter is customized to 50 bp read length for hg38; 
#if you do not trim reads to 25 bp or are not working with the hg38 assembly, this parameter will need to be adjusted accordingly. 
