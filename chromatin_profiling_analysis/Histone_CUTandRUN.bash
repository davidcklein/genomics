...
for f in *R1_trim25.fastq.gz; do echo $f; if [[ ! -e "${f/_R1_trim25.fastq.gz/_bt2.sam}" ]]; then bowtie2 -q -N 1 -p 8 -X 1000 -I 100 --very-sensitive --no-mixed --no-discordant --no-unal -x $hg38 -1 $f -2 ${f/R1/R2} -S ${f/_R1_trim25.fastq.gz/_bt2.sam}; fi; done
#Bowtie2 will align fragments between 100 and 1000 bp, leaving unaligned reads out of the output sam files

mv *_bt2.sam ../bam/
cd ../bam/

for f in *_bt2.sam; do if [[ ! -e "${f/_bt2.sam/_filtered.sam}" ]]; then samtools view -@ 7 -h -Sq 10 -o ${f/_bt2.sam/_filtered.sam} $f; fi; done
...
for f in *_dup_marked.sam; do if [[ ! -e "${f/_dup_marked.sam/.150_500.sam}" ]]; then awk ' $9 <= 500 && $9 >= 150 || $9 >= -500 && $9 <= -150 ' $f > ${f/_dup_marked.sam/.150_500.sam}; \
cp $hg38_header ${f/_dup_marked.sam/.150_500.header}; cat ${f/_dup_marked.sam/.150_500.sam} >> ${f/_dup_marked.sam/.150_500.header}; rm ${f/_dup_marked.sam/.150_500.sam}; \
mv ${f/_dup_marked.sam/.150_500.header} ${f/_dup_marked.sam/.150_500.sam}; samtools view -@ 7 -S -t $hg38_cs -b -o ${f/_dup_marked.sam/.150_500.bam} ${f/_dup_marked.sam/.150_500.sam}; fi; done

for f in *150_500.bam; do if [[ ! -e "${f/.bam/_sorted.bam.bai}" ]]; then samtools sort -@ 7 -O BAM -o ${f/.bam/_sorted.bam} $f; samtools index -@ 7 ${f/.bam/_sorted.bam}; fi; done
...
for f in *sorted.bam; do if [[ ! -e "${f/_sorted.bam/.bw}" ]]; then bamCoverage --exactScaling --centerReads -b $f -of bigwig -bs 5 --smoothLength 20 -p 8 --normalizeUsing RPGC --effectiveGenomeSize 2701495761 -e -o ${f/_sorted.bam/.bw}; fi; done
